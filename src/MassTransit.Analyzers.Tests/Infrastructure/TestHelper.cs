...
namespace MassTransit.Analyzers.Tests.Infrastructure
{
	internal static class TestHelper
	{
		...
		static TestHelper()
		{
			var referencedAssemblies = typeof(TestHelper).Assembly.GetReferencedAssemblies()
				.Concat(typeof(AwaitOnRequestAnalyzer).Assembly.GetReferencedAssemblies())
				.Concat(typeof(IConsumer).Assembly.GetReferencedAssemblies())
				.Concat(new[] { typeof(Uri).Assembly.GetName(), typeof(object).Assembly.GetName() });
